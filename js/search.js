...
// 賽程與比賽狀態統一由 live-boxscore.json 提供。
...
async function loadAllData() {
  const [teamData, liveData] = await Promise.all([
  ...
  return schedules
    .filter(game => {
      ...
      const text = normalize([
        ...
        meta.date || game.date,
        meta.time || game.time,
        meta.venue || game.venue,
        meta.type || game.type,
        meta.typeText,
        meta.status || game.status,
        meta.statusText,
        meta.home || game.teams?.home || game.home,
        meta.away || game.teams?.away || game.away,
        meta.win,
        meta.lose,
        meta.save,
        meta.mvp
    ...
    .map(game => {
      const meta = game.meta || {};
      const home = meta.home || game.teams?.home || game.home || "—";
      const away = meta.away || game.teams?.away || game.away || "—";
      const gameSno = game.gameSno || game.id || "";
      const date = meta.date || game.date || "";
      const time = meta.time || game.time || "";
      const venue = meta.venue || game.venue || "";
      const statusText = meta.statusText || meta.status || game.statusText || game.status || "狀態未定";

      return {
        type: "schedule",
        title: `${away} vs ${home}`,
        subtitle: `${date || "日期未定"}｜${statusText}｜${venue || time || "資訊待定"}`,
        href: gameSno
          ? `match.html?gameSno=${encodeURIComponent(gameSno)}`
          : `schedule.html?date=${encodeURIComponent(date)}`,
        badge: "比賽"
      };
    });
}
...
function renderResults(results) {
  ...
  searchResults.innerHTML = `
    ...
    ${renderGroup("比賽", grouped.schedule)}
  `;
}
