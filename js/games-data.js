...
function syncLocalStorage(games) {
  ...
  try {
    // 只保存離線備援真正需要的精簡欄位，避免完整 boxscore 撐爆 quota。
    const compactMap = {};

    games.forEach(g => {
      if (g.gameSno == null) return;

      compactMap[g.gameSno] = {
        gameSno: g.gameSno,
        meta: {
          date: g.date,
          home: g.home,
        ...
        totals: {
          home: { R: g.homeScore, H: g.homeH, E: g.homeE },
          away: { R: g.awayScore, H: g.awayH, E: g.awayE }
        }
      };
    });

    localStorage.setItem(LOCAL_BOX_KEY, JSON.stringify(compactMap));
  } catch (err) {
    console.warn("⚠️ localStorage 比賽快取寫入失敗，已略過：", err);
