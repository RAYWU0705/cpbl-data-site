...
/* =========================================================
   ...
   v5.4.4-FINAL-VUE-RESCUE-TARGETED-MERGE
...
const VERSION = "v5.4.4-FINAL-VUE-RESCUE-TARGETED-MERGE";
...
async function main() {
  ...
  for (const target of targets) {
    ...
    if (MERGE_EACH && !SKIP_MERGE) {
      const mergeStep = await runMerge([target]);
  ...
  if (!MERGE_EACH && !SKIP_MERGE) {
    ...
    const mergeStep = await runMerge(targets);
...
async function runMerge(targets = []) {
  const gameList = targets
    .map(target => String(target?.gameSno || "").trim())
    .filter(Boolean)
    .join(",");
