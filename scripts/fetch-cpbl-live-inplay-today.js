...
function looksLikeClockScore(awayScore, homeScore, game = {}) {
  const a = Number(awayScore);
  const h = Number(homeScore);

  if (!Number.isFinite(a) || !Number.isFinite(h)) return false;

  const time = cleanOneLine(game.time || game.meta?.time || "");

  if (/^\d{1,2}:\d{2}$/.test(time)) {
    const [hh, mm] = time.split(":").map(Number);
    if (a === hh && h === mm) return true;
  }

  if (a >= 13 && h >= 20) return true;

  return false;
}

function sanitizeHomeLiveCardScore(liveCard, scheduleGame) {
  if (!liveCard) return null;

  if (looksLikeClockScore(liveCard.awayScore, liveCard.homeScore, scheduleGame)) {
    return {
      ...liveCard,
      awayScore: null,
      homeScore: null,
      scoreRejected: true,
      scoreRejectReason: "looks-like-game-time"
    };
  }
...
function hasOfficialLiveSignal(game = {}, liveCard = null, detailBundle = null) {
  const rawStatus = cleanOneLine(game.rawStatus || "");
  const statusText = cleanOneLine(game.statusText || "");
  const detailStatusText = cleanOneLine(detailBundle?.meta?.statusText || "");
  const liveState = detailBundle?.detail?.liveState || null;

  if (game.status === "live") return true;
  if (isLiveText(rawStatus) || isLiveText(statusText) || isLiveText(detailStatusText)) return true;

  if (liveState?.inningText && isStrictInningText(liveState.inningText)) return true;

  if (
    liveCard &&
    (
      liveCard.isLive === true ||
      isLiveText(liveCard.statusText || "") ||
      isLiveText(liveCard.debugText || "")
    )
  ) {
    return true;
  }
...
function isOnlyPregameZeroScore(game = {}, liveCard = null) {
  const rawStatus = cleanOneLine(game.rawStatus || "");
  const statusText = cleanOneLine(game.statusText || "");

  const scheduleSaysScheduled =
    game.status === "scheduled" ||
    isScheduledText(rawStatus) ||
    isScheduledText(statusText);

  if (!scheduleSaysScheduled) return false;

  const hasOnlyZeroRuns =
  ...
  const liveCardHasNoScore =
    !liveCard ||
    (
      liveCard.awayScore === null &&
      liveCard.homeScore === null
    );

  return hasOnlyZeroRuns && liveCardHasNoScore;
}

function shouldEnterLiveMode(game, liveCard = null, detailBundle = null) {
  ...
  const safeLiveCard = sanitizeHomeLiveCardScore(liveCard, game);

  if (isOnlyPregameZeroScore(game, safeLiveCard)) return false;

  if (hasOfficialLiveSignal(game, safeLiveCard, detailBundle)) return true;

  const hasRealScore =
    (
      typeof game.awayScore === "number" &&
      typeof game.homeScore === "number" &&
      !looksLikeClockScore(game.awayScore, game.homeScore, game)
    ) &&
    (
      game.awayScore > 0 ||
      game.homeScore > 0 ||
      ...
      typeof game.awayE === "number" ||
      typeof game.homeE === "number"
    );

  return isAfterGameStart(game.date, game.time) && hasRealScore;
}
...
function parseLiveInningNumberFromText(text = "") {
  const s = cleanOneLine(text);
  ...
  const m2 = s.match(/^([一二三四五六七八九十]+)\s*局\s*[上下]$/);

  if (m2 && zh[m2[1]]) return zh[m2[1]];

  return null;
}

function isUsefulLineScoreCell(value) {
  if (value === "X") return true;
...
function normalizeLineScoreCell(value) {
...
function normalizeAsymmetricLiveLineScore(lineScore = {}, liveState = null) {
  const inning = parseLiveInningNumberFromText(liveState?.inningText || "");
  const half = liveState?.half || "";

  const awayRaw = Array.isArray(lineScore.away) ? lineScore.away : [];
  const homeRaw = Array.isArray(lineScore.home) ? lineScore.home : [];

  const away = awayRaw.map(normalizeLineScoreCell);
  const home = homeRaw.map(normalizeLineScoreCell);
  ...
  let awayLimit = inning;
  let homeLimit = inning;

  if (half === "top") {
    // 3局上：客隊第3局正在打，主隊最多只完成到第2局。
    homeLimit = Math.max(inning - 1, 0);
  }

  const outAway = away.slice(0, awayLimit);
  const outHome = home.slice(0, homeLimit);

  // 直播中官方常會只吐已完成的主隊欄位，例如 3局上主隊只有第1局資料。
  // 不補假 0；只保留官方已吐出的格子。
  return {
    away: outAway,
    home: outHome
...
function hasAsymmetricLiveLineScore(lineScore = {}) {
...
function trimLiveLineScoreToCurrentInning(lineScore = {}, liveState = null) {
  const inning = parseLiveInningNumberFromText(liveState?.inningText || "");
  ...
  if (!inning || inning < 1) {
    const lastUseful = Math.max(
      away.findLastIndex(isUsefulLineScoreCell),
      home.findLastIndex(isUsefulLineScoreCell)
    );
    ...
    return {
      away: away.slice(0, lastUseful + 1),
      home: home.slice(0, lastUseful + 1)
    };
  }

  const limit = Math.min(Math.max(inning, 1), 15);

  return {
    away: away.slice(0, limit),
    home: home.slice(0, limit)
  };
}


function buildInferredLiveLineScore(liveState, totals = emptyTotals()) {
  const inning = parseLiveInningNumberFromText(liveState?.inningText || "");

  // 只有第一局能安全推論。第二局以後不能把總分誤塞成單局分數。
  if (inning !== 1) {
    return {
      away: [],
      home: []
  ...
  const awayR = numberOrNull(totals?.away?.R);
  const homeR = numberOrNull(totals?.home?.R);
...
function normalizeLiveLineScoreForWrite(lineScore = {}, liveState = null, totals = emptyTotals(), source = "") {
  const asymmetric = normalizeAsymmetricLiveLineScore(lineScore, liveState);

  if (hasAsymmetricLiveLineScore(asymmetric)) {
    return {
      lineScore: asymmetric,
      source: source || "boxscore-linescore-asymmetric-trimmed",
      qualityHint: source && (source.includes("official-text-scoreboard") || source.includes("official-text-scoreboard-newline") || source.includes("vue"))
        ? "confirmed"
        : "partial"
    };
  }

  const inferred = buildInferredLiveLineScore(liveState, totals);

  if (hasAsymmetricLiveLineScore(inferred)) {
    return {
      lineScore: inferred,
      source: "inferred-first-inning-live-state",
      qualityHint: "partial"
    };
  }
...
function buildDataQuality(game) {
  ...
  const hasLineScore = hasAsymmetricLiveLineScore(game?.lineScore || {});
  const lineScoreQualityHint = game?.lineScoreQualityHint || "";
  const hasInferredLineScore =
    game?.lineScoreSource === "inferred-first-inning-live-state" ||
    lineScoreQualityHint === "partial";
  ...
  return {
    ...
    lineScore: lineScoreQualityHint === "confirmed" ? "confirmed" : hasInferredLineScore ? "partial" : getQualityLevel(hasLineScore),
  ...
  const payload = await page.evaluate((scheduleGamesInPage) => {
    ...
    function parseScoreFromText(text, game) {
      ...
      const scheduledText =
        ...
        t.includes("未開賽") ||
        t.includes("尚未開打");
      const timeText = one(game.time || "");

      function rejectClock(a, h) {
        if (/^\d{1,2}:\d{2}$/.test(timeText)) {
          const [hh, mm] = timeText.split(":").map(Number);
          if (Number(a) === hh && Number(h) === mm) return true;
        }

        return Number(a) >= 13 && Number(h) >= 20;
      }
      ...
      for (const pattern of patterns) {
        ...
        if (found && !rejectClock(found[1], found[2])) {
          return {
            awayScore: Number(found[1]),
      ...
      return {
        ...
        homeScore: null,
        rejected: false,
        reason: "no-safe-score"
      };
    }

    function tIncludesLive(text) {
      const t = one(text);
  ...
  */
  ...
  return await page.evaluate((TEAM_NAMES_IN_PAGE, targetGameSno, entryMode, entryUrl, sideHintInPage, expectedGameInPage) => {
    ...
    function extractLiveStateScout(pageMeta, batters, pitchers) {
      ...
      const recentEvents = splitEvents(recentEventsText);
      const confidence = batter && pitcher ? "confirmed" : "debug";

      return {
        source: "boxscore-live-state-write",
        quality: confidence,
        inningText,
        inning: inningText,
        half,
        ...
        battingTeam,
        fieldingTeam,
        offenseTeam: battingTeam,
        defenseTeam: fieldingTeam,
        batter,
        pitcher,
        ...
        message: batter && pitcher
          ? "LIVE state write：已從官方 boxscore 文字寫入目前投打"
          : "LIVE state write：已偵測局數，但目前投打仍需人工確認"
      };
    }


    function findVueInstanceForLineScore() {
    ...
    function pickVueValue(row, candidates) {
      ...
      for (const key of keys) {
        ...
        if (candidates.some(candidate => lower === candidate.toLowerCase())) {
          return row[key];
        }
      }
    ...
    function normalizeVueScoreCell(value) {
    ...
    function findVueLineScoreFromScoreboards() {
      const vm = findVueInstanceForLineScore();

      if (!vm) {
        return {
          ok: false,
          source: "vue-not-found",
          lineScore: {
            away: [],
            home: []
          },
      ...
      for (const key of sourceKeys) {
        if (Array.isArray(vm[key]) && vm[key].length) {
          board = vm[key];
          usedKey = key;
          break;
        }
      ...
      for (const row of rows) {
        ...
        const awayRaw = pickVueValue(row, [
          "VisitingScore",
          "VisitingPoint",
          "VisitingTeamScore",
          "AwayScore",
        ...
        const homeRaw = pickVueValue(row, [
          "HomeScore",
          "HomePoint",
          "HomeTeamScore",
          "HScore",
        ...
        const awayCell = normalizeVueScoreCell(awayRaw);
        const homeCell = normalizeVueScoreCell(homeRaw);

        if (inning && inning >= 1 && inning <= 15) {
          away[inning - 1] = awayCell;
          home[inning - 1] = homeCell;
        } else {
          away.push(awayCell);
          home.push(homeCell);
        }
      ...
      const cleanAway = away.map(v => v ?? "");
      const cleanHome = home.map(v => v ?? "");

      return {
        ok: hasUsefulLineRow(cleanAway) || hasUsefulLineRow(cleanHome),
        source: `vue-${usedKey}`,
        lineScore: {
          away: cleanAway,
          home: cleanHome
    ...
    function parseScoreboardFromOfficialText(pageMeta = {}) {
      // 重要：不能先用 one(document.body.innerText)，那會把換行吃掉，
      // SCOREBOARD 區塊會整坨變成一行，導致找不到 1~9 局 header。
      ...
      const scoreboardIndex = lines.findIndex(line => line === "SCOREBOARD");

      if (scoreboardIndex < 0) {
        return {
          ok: false,
          source: "official-text-scoreboard-not-found",
          lineScore: {
            away: [],
            home: []
          },
          debug: {
            reason: "scoreboard-not-found",
      ...
      function findLineIndexFrom(start, predicate, max = 260) {
      ...
      function isInningHeaderAt(index) {
        const slice = lines.slice(index, index + 9);

        if (slice.length < 9) return false;

        return slice.every((value, i) => value === String(i + 1));
      }

      function normalizeScoreCell(v) {
        const s = one(v);
      ...
      const headerIndex = findLineIndexFrom(
        scoreboardIndex,
        (_line, i) => isInningHeaderAt(i)
      );

      if (headerIndex < 0) {
        return {
          ok: false,
          source: "official-text-inning-header-not-found",
          lineScore: {
            away: [],
            home: []
          },
          debug: {
            reason: "inning-header-not-found",
            lineCount: lines.length,
            sample: lines.slice(scoreboardIndex, scoreboardIndex + 100)
          }
        };
      }

      const rIndex = findLineIndexFrom(
        headerIndex + 9,
        (line, i) =>
          line === "R" &&
          lines[i + 1] === "H" &&
      ...
      const scoreZoneEnd = rIndex > 0 ? rIndex : Math.min(lines.length, headerIndex + 50);
      const rawScoreZone = lines.slice(headerIndex + 9, scoreZoneEnd);

      // 官方文字抽出會把空白局消掉，只留下已打到的逐局分數。
      const scoreCells = rawScoreZone
        .map(normalizeScoreCell)
        .filter(value => value !== null);

      // 依 liveState 判斷目前上下半局，比分列如何切：
      // 2局上：客隊可能有 2 格、主隊通常只有 1 格
      // 2局下：客隊有 2 格、主隊可能有 1~2 格
      const liveText = one(
        document.querySelector(".game_info, .ScoreBoard, .scoreboard")?.innerText || rawText
      );

      const inningMatch =
        liveText.match(/(\d+)\s*局\s*([上下])/) ||
        rawText.match(/(\d+)\s*局\s*([上下])/);

      const currentInning = inningMatch ? Number(inningMatch[1]) : null;
      const halfText = inningMatch ? inningMatch[2] : "";

      let awayCount = null;
      let homeCount = null;

      if (currentInning && halfText === "上") {
        awayCount = currentInning;
        homeCount = Math.max(currentInning - 1, 0);
      } else if (currentInning && halfText === "下") {
        awayCount = currentInning;
        homeCount = currentInning;
      }

      let away = [];
      let home = [];

      if (awayCount !== null && homeCount !== null) {
        away = scoreCells.slice(0, awayCount);
        home = scoreCells.slice(awayCount, awayCount + homeCount);
      } else if (scoreCells.length >= 2) {
        const split = Math.ceil(scoreCells.length / 2);

        away = scoreCells.slice(0, split);
        home = scoreCells.slice(split);
      } else if (scoreCells.length === 1) {
        away = [scoreCells[0]];
        home = [];
      }

      const ok = away.length > 0 || home.length > 0;

      return {
        ok,
        source: "official-text-scoreboard-newline",
        lineScore: {
          away,
          home
        },
        debug: {
          ...
          away,
          home,
          sample: lines.slice(scoreboardIndex, Math.min(lines.length, scoreboardIndex + 110))
        }
      };
    }
    ...
    function parseDetail(pageMeta) {
      ...
      const tableLineScoreBundle = findLineScoreFromTables(tables);
      const vueLineScoreBundle = findVueLineScoreFromScoreboards();
      const textScoreboardBundle = parseScoreboardFromOfficialText(pageMeta);

      const lineScoreBundle =
        textScoreboardBundle.ok
          ? {
            tableIndex: null,
            rheTableIndex: tableLineScoreBundle.rheTableIndex,
            lineScore: textScoreboardBundle.lineScore,
            totals: tableLineScoreBundle.totals,
            count:
              (textScoreboardBundle.lineScore.away?.filter(v => v !== "").length || 0) +
              (textScoreboardBundle.lineScore.home?.filter(v => v !== "").length || 0),
            source: textScoreboardBundle.source
          }
          : vueLineScoreBundle.ok
            ? {
              tableIndex: null,
              rheTableIndex: tableLineScoreBundle.rheTableIndex,
              lineScore: vueLineScoreBundle.lineScore,
              totals: tableLineScoreBundle.totals,
              count:
                (vueLineScoreBundle.lineScore.away?.filter(v => v !== "").length || 0) +
                (vueLineScoreBundle.lineScore.home?.filter(v => v !== "").length || 0),
              source: vueLineScoreBundle.source
            }
            : tableLineScoreBundle;
      ...
      return {
        ...
        debug: {
          ...
          vueLineScoreParsed: vueLineScoreBundle,
          textScoreboardParsed: textScoreboardBundle,
          tableLineScoreParsed: tableLineScoreBundle,
          textScoreboardParsed: textScoreboardBundle,
...
function rescueLineScoreAfterOverride(game, detailBundle, scheduleGame) {
  if (!game) return game;

  const currentHasLineScore = hasAsymmetricLiveLineScore(game.lineScore || {});
  if (currentHasLineScore) return game;

  const detail = detailBundle?.detail || {};
  const detailHasLineScore = hasAsymmetricLiveLineScore(detail.lineScore || {});

  if (!detailHasLineScore) return game;

  const normalized = normalizeLiveLineScoreForWrite(
    detail.lineScore,
    game.liveState || detail.liveState || null,
    game.totals || detail.totals || emptyTotals(),
    detail?.debug?.lineScoreParsed?.source || "detail-lineScore-rescue"
  );

  if (!hasAsymmetricLiveLineScore(normalized.lineScore || {})) return game;

  const rescued = {
    ...game,
    lineScore: normalized.lineScore,
    lineScoreSource: `${normalized.source || "detail-lineScore-rescue"}+after-override-rescue`,
    lineScoreQualityHint: normalized.qualityHint || "partial",
    debug: {
      ...(game.debug || {}),
      liveInplay: {
        ...(game.debug?.liveInplay || {}),
        lineScoreRescueAfterOverride: {
          enabled: true,
          gameSno: Number(scheduleGame?.gameSno || game.gameSno || 0),
          reason: "merged lineScore was empty after override / final merge, restored from detailBundle.detail.lineScore",
          source: normalized.source || "detail-lineScore-rescue",
          awayCount: normalized.lineScore.away?.length || 0,
          homeCount: normalized.lineScore.home?.length || 0,
          liveState: game.liveState?.inningText || detail.liveState?.inningText || ""
        }
      }
    }
  };
  ...
  return {
    ...rescued,
    dataQuality: buildDataQuality(rescued)
  };
}
...
function createOrMergeGame(oldGame, scheduleGame, liveCard, detailBundle) {
  ...
  const lineScoreNormalize = normalizeLiveLineScoreForWrite(
    lineScore,
    liveState,
    totals,
    detail?.debug?.lineScoreParsed?.source || "boxscore-table"
  );

  const finalLineScore = lineScoreNormalize.lineScore;
  const lineScoreSource = lineScoreNormalize.source;
  const lineScoreQualityHint = lineScoreNormalize.qualityHint;
  ...
  const mergedGame = {
    ...
    lineScore: finalLineScore,
    lineScoreSource,
    lineScoreQualityHint,
    totals,
    batters,
    pitchers,
...
async function main() {
  ...
  try {
    ...
    const liveCardMap = new Map(
      liveCards.map(card => [
        Number(card.gameSno),
        sanitizeHomeLiveCardScore(
          card,
          scheduleGames.find(g => Number(g.gameSno) === Number(card.gameSno)) || null
        )
      ])
    );
    ...
    for (const game of inplayGames) {
      ...
      const beforeLineScoreRescue =
        hasAsymmetricLiveLineScore(merged.lineScore || {});

      merged = rescueLineScoreAfterOverride(merged, detailBundle, game);

      if (!beforeLineScoreRescue && hasAsymmetricLiveLineScore(merged.lineScore || {})) {
        console.log(
          `   🧯 lineScore rescue：從 detailBundle 救回逐局｜` +
          `客${merged.lineScore?.away?.length || 0}/主${merged.lineScore?.home?.length || 0}｜` +
          `${merged.lineScoreSource || "rescue"}`
        );
      }
      ...
      if (merged.liveState?.source === "boxscore-live-state-scout" || merged.liveState?.source === "boxscore-live-state-write") {
        console.log(
          `   🛰️ LIVE state write：局數=${merged.liveState.inningText || "—"}｜` +
