...
const VERSION = "v5.5.1-FINAL-NO-DOWNGRADE-GUARD";
...
function getStatusFromSchedule(g) {
  ...
  const rawStatus = String(g.rawStatus || "");
  const note = String(g.note || g.remark || "");
  const specialText = [statusText, time, duration, rawTime, rawStatus, note].join(" ");

  // 特殊狀態必須優先於 0:0 等數字比分判斷。
  // 保留比賽在官方賽程頁可能仍帶有數字比分，不能因此誤判 FINAL。
  if (/保留|續賽|中止/.test(specialText)) return "suspended";

  if (
    /延賽|時間未定|因雨延賽/.test(specialText)
  ) {
    return "postponed";
  }

  if (/取消/.test(specialText)) return "cancelled";
...
function isProtectedStatus(status) {
  return ["suspended", "postponed", "cancelled"].includes(String(status || "").toLowerCase());
}

function resolveScheduleStatus(oldGame, scheduleGame) {
  const parsed = getStatusFromSchedule(scheduleGame);
  const oldStatus = String(oldGame?.meta?.status || oldGame?.status || "").toLowerCase();

  const zeroZero =
    Number(scheduleGame?.awayScore) === 0 &&
    Number(scheduleGame?.homeScore) === 0;

  const completionText = [
    scheduleGame?.statusText,
    scheduleGame?.duration,
    scheduleGame?.decisionLine,
    scheduleGame?.rawStatus
  ].map(v => String(v || "")).join(" ");

  const hasCompletionSignal = /比賽結束|完賽|FINAL|勝投|敗投|MVP/i.test(completionText);

  // 已知特殊場次遇到模糊的 0:0，不允許被整季賽程更新洗回 FINAL。
  if (isProtectedStatus(oldStatus) && parsed === "final" && zeroZero && !hasCompletionSignal) {
    return oldStatus;
  }

  if (oldStatus === "final" && parsed !== "final") {
    return "final";
  }
...
async function writeGames(games) {
  ...
  if (await fileExists(DATA_FILE)) {
    ...
    const backup = path.join(
      BACKUP_DIR,
      `live-boxscore-before-schedule-score-${stamp}.json`
    );
    await fs.copyFile(DATA_FILE, backup);
    console.log("🛡️ 已備份：", path.relative(process.cwd(), backup));
  }
...
function mergeScheduleScore(oldGame, g) {
  ...
  const status = resolveScheduleStatus(oldGame, g);
  const decisions = parseDecisionLine(g.decisionLine);
  ...
  return {
    ...
    status,
    statusText: getStatusText(status),
    finalLock: status === "final" ? oldGame.finalLock : undefined,
    ...
    meta: {
      ...
      status,
      statusText: getStatusText(status),
      finalLock: status === "final" ? oldMeta.finalLock : undefined,
      finalLockSource: status === "final" ? oldMeta.finalLockSource : undefined,
      finalVueEnhanced: status === "final" ? oldMeta.finalVueEnhanced : undefined,
      finalVueForceMerged: status === "final" ? oldMeta.finalVueForceMerged : undefined,
      finalVueForcedFromStatus: status === "final" ? oldMeta.finalVueForcedFromStatus : undefined,
...
async function main() {
  ...
  const beforeFinal = games.filter(g =>
    String(g?.meta?.status || g?.status || "").toLowerCase() === "final"
  ).length;

  const afterFinal = result.filter(g =>
    String(g?.meta?.status || g?.status || "").toLowerCase() === "final"
  ).length;

  if (
    beforeFinal >= 20 &&
    (afterFinal < beforeFinal - 3 || afterFinal < beforeFinal * 0.9)
  ) {
    throw new Error(
